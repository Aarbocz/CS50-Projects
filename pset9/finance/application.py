...
def buy():
    ...
    if request.method == "POST":
        ...
        # Update portfolio
            ...
            old_quantity = db.execute("SELECT quantity FROM portfolio WHERE id = ? AND symbol = ?",
                                      session["user_id"], stocks['symbol'])
            new_quantity = old_quantity[0]['quantity'] + int(quantity)
            db.execute("UPDATE portfolio SET quantity = ? WHERE id = ? and symbol = ?",
                       new_quantity, session["user_id"], stocks['symbol'])
